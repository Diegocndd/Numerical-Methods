...
def x_s(ini, fim, s):
    return ((ini + fim) / 2.0) + (((fim - ini) / 2.0) * math.tanh((math.pi/2) * math.sinh(s)))

def s_barra(ini, fim, s):
    return ((fim + ini) / 2.0) + (((fim - ini) / 2.0) * s)

def f_barra(ini, fim, s):
    return f(x_s(ini, fim, s)) * ((fim - ini) / 2) * ((math.pi / 2) * (math.cosh(s) / math.pow((math.cosh((math.pi / 2) * math.sinh(s))), 2)))

def gaussLegendreExpDupla(ini, fim, iniC, fimC, qtdPontos):
    resultado = 0.0
    w1 = w2 = w3 = None
    s1 = s2 = s3 = None
...
def GaussLegendreParticoesExpDupla(ini, fim, iniC, fimC, qtdPontos, eps):
    integralNova = 1e10
    integralVelha = 0.0
    ...
    while (abs((integralNova - integralVelha) / integralNova) > eps):
        ...
        for i in range(0, N):
            xIn = iniC + (i * deltaX)
            xFim = xIn + deltaX
            integralNova += gaussLegendreExpDupla(ini, fim, xIn, xFim, qtdPontos)
            # print(integralNova)
        N *= 2
...
while (abs((resultNovo - resultVelho) / resultNovo) > eps):
    ...
    resultNovo = GaussLegendreParticoesExpDupla(eIni, eFim, -c, c, qtdPontos, eps)
